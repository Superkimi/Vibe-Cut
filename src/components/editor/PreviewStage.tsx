...
function MediaLayer({
  ...
  const style = {
    ...
    width: `${(clip.transform.width / projectWidth) * 100}%`,
    height: `${(clip.transform.height / projectHeight) * 100}%`,
    opacity: clip.opacity,
    transform: `rotate(${clip.transform.rotation}deg) scale(${clip.transform.scaleX}, ${clip.transform.scaleY})`,
    filter: `brightness(${1 + clip.adjustments.brightness}) contrast(${1 + clip.adjustments.contrast}) saturate(${1 + clip.adjustments.saturation}) blur(${clip.adjustments.blur}px)`,
    "--fit": clip.fit,
